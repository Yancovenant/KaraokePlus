...
from kplus.pipelines.audio.plotter import PlotterSimple
...
class Refiner:
    def __init__(self, **options):
        self.sr = 16000 # maybe?
        if env.verbose:
            self.plotter = PlotterSimple(options.pop("resample", False), shared_xaxes=True, vertical_spacing=0.05)
    ...
    def _compute_optimal(text: TextTiming, *ai_text_list, *, mel: Mel, safe_start: float) -> TextTiming:
        ...
        for i, (*word_list,) in enumerate(zip(*(ai_text.words for ai_text in ai_text_list))):
            ...
            if best_score is not None:
                ...
                if (future_valleys := valley_times[valley_times > mid + 0.040]).size > 0:
                    best_end = future_valleys[np.abs(future_valleys - best_end).argmin()]
                if (past_valleys := valley_times[valley_times < mid - 0.040]).size > 0:
    ...
    def _compute_unreliable(text: TextTiming, *ai_text_list, *, mel: Mel, safe_start: float) -> TextTiming:
        ...
        for i, (*word_list,) in enumerate(zip(*(ai_text.words for ai_text in ai_text_list))):
            ...
            if best_score is not None:
                ...
                if i < len(text.words) - 1 and future_start := min(w.start for ai_text in ai_text_list for w in ai_text.words[i+1:]):
    ...
    def _refine(self, o_text: TextTiming, *ai_text_list, *, safe_start: float, audio_result: DetectionResult) -> TextTiming:
        refined_text = copy.deepcopy(ori_seg)
