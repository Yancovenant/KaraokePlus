    ...
    def asr2ref(self,
        ...
        **kwargs
        ...
        for i in range(len(audiosegments)):
            last_audio_and_still_have_word_missing = (i == last_audio_id and (missing := _check_missing(len(ref_tokens))))
            if last_audio_and_still_have_word_missing:
                logger.debug(f"Subject already in a last segment, but there is still missing word {missing}")
                prev_anchor_audio_id = max(k for k, v in ref_by_audio.items() if v)
                _, prev_end_gap = unused_gaps[prev_anchor_audio_id]
                if prev_end_gap > 0.4:
                    logger.debug(f"  Previous end gap is still large, assigned it to previous {ref_by_audio[prev_anchor_audio_id]} | {missing}")
                    ref_by_audio[prev_anchor_audio_id] = sorted(ref_by_audio[prev_anchor_audio_id] + missing)
                else:
                    logger.debug("  Previous end gap insufficient, try getting the next audio")
                    if prev_anchor_audio_id < i:
                        logger.debug(f"  Next audio exists, {prev_anchor_audio_id+1} | {ref_by_audio[prev_anchor_audio_id+1]}")
                        ref_by_audio[prev_anchor_audio_id+1] = sorted(missing)
            token_ids = ref_by_audio[i]
            if not token_ids: continue
            there_still_prev_word_being_skipped = (min(token_ids) > last_word_idx + 1 and (missing := _check_missing(min(token_ids))))
            if there_still_prev_word_being_skipped:
                logger.debug(f"Subject still skipping words: {ref_by_audio[i-1]} | {missing} | {ref_by_audio[i]}")
                ...
                if prev_anchor_audio_id := next((j for j in range(i - 1, -1, -1) if ref_by_audio[j]), None):
                    logger.debug(f"  Previous Anchor Audio Exists: {prev_anchor_audio_id}")
                    _, anchor_end_gap = unused_gaps[prev_anchor_audio_id]
                    if anchor_end_gap > 1.0:
                        logger.debug(f"  Previous anchor gap still high assigned to previous anchor {sorted(ref_by_audio[prev_anchor_audio_id] + missing)}")
                        ref_by_audio[prev_anchor_audio_id] = sorted(ref_by_audio[prev_anchor_audio_id] + missing)
                    else:
                        logger.debug(f"  Previous anchor gap insufficient, combining to previous audio: {ref_by_audio[i-1]} | {missing} | {token_ids}")
                        ref_by_audio[i] = sorted(token_ids + missing + ref_by_audio[i-1])
                        ref_by_audio[i-1] = sorted(token_ids + missing + ref_by_audio[i-1])
                elif (ratio:=similarity(missing_text, known_text)) > 0.7:
                    logger.debug(f"  Similarity high ({ratio}) between\n{missing_text} | {known_text}\n{sorted(token_ids + missing)}")
                    ref_by_audio[i] = sorted(token_ids + missing)
                else:
                    logger.debug(f"  Combining to previous Audio: {ref_by_audio[i-1]} | {missing} | {token_ids}")
                    ref_by_audio[i] = sorted(token_ids + missing + ref_by_audio[i-1])
                    ref_by_audio[i-1] = sorted(token_ids + missing + ref_by_audio[i-1])
            token_ids = ref_by_audio[i]
            ...
            if i < last_audio_id and (next_token_ids := ref_by_audio[i+1]):
                ...
                if missing := sorted(set(range(max(token_ids) + 1, min(next_token_ids)))):
                    logger.debug(f"Subject is missing: {token_ids} | {missing} | {next_token_ids}")
                    if next_start_gap > end_gap:
                        logger.debug(f"  Belong to the next: {sorted(next_token_ids + missing)}")
                        ref_by_audio[i+1] = sorted(next_token_ids + missing)
                    else:
                        logger.debug(f"  Combine Both: {sorted(token_ids + missing + next_token_ids)}")
                        ref_by_audio[i] = sorted(token_ids + missing + next_token_ids)
                elif end_gap > 2.0 or next_start_gap > 1.0:
                    # No Unknown but the gap in either each segment is so long
                    # So we combine
                    logger.debug(f"Subject for combine: {ref_by_audio[i]} | {ref_by_audio[i+1]}")
                    ref_by_audio[i] = sorted(ref_by_audio[i] + ref_by_audio[i+1])
                    ref_by_audio[i+1] = sorted(ref_by_audio[i] + ref_by_audio[i+1])
            token_ids = ref_by_audio[i]
            last_word_idx = max(token_ids)

        assigned = [token_id for token_ids in ref_by_audio.values() for token_id in token_ids]
        missing = (set(range(len(ref_tokens))) - set(assigned))
        if missing:
