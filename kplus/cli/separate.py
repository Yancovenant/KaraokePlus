...
class Separate(Command):
    """ Separate either input URL or
        file path into 2 different stems.
    ...
    def _parse_config(self, args):
        ...
        if opt.demucs and opt.uvr:
            raise ValueError((
                "Cannot use Demucs model and "
                "UVR model at the same time: %s - %s"
            ), opt.demucs, opt.uvr)
        if not opt.demucs and not opt.uvr:
            raise ValueError(
                "Either ``--demucs MODEL`` or "
                "``--uvr MODEL`` need's to be "
                "passed at runtime"
            )
    ...
    def run(self, args):
        ...
        logger.info(rich.Panel(
            table,
            title=f"Finished separating {info.filepath!s}"
        ))
